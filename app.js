/* ================= APP CONFIGURATION (RESTORED & SAFE) ================= */
var AppConfig = window.AppConfig || {
    appName: 'InvoiceStatePro',
    version: '1.0.0',
    debugMode: false,
...
/* ================= GLOBAL PROXY BINDINGS (100% FIXED FOR ALL BUTTONS) ================= */
// اب تمام فنکشنز ڈائریکٹ ڈکلیئر کیے گئے ہیں تاکہ HTML بٹنز انہیں فورا کیچ کر سکیں
function toggleSidebar() { if(typeof UIManager !== 'undefined') UIManager.toggleSidebar(); }
window.toggleSidebar = toggleSidebar;

function switchTab(tabId, event) { if(typeof UIManager !== 'undefined') UIManager.switchTab(tabId, event); }
window.switchTab = switchTab;

function toggleDarkMode() { if(typeof UIManager !== 'undefined') UIManager.toggleDarkMode(); }
window.toggleDarkMode = toggleDarkMode;

function showToast(msg, type) { if(typeof NotificationManager !== 'undefined') NotificationManager.show(msg, type); }
window.showToast = showToast;

function setLanguage(lang) { if(typeof I18nManager !== 'undefined') I18nManager.setLanguage(lang); }
window.setLanguage = setLanguage;

function saveDraft() { if(typeof UIManager !== 'undefined') UIManager.saveDraft(); }
window.saveDraft = saveDraft;

function saveFinal() { if(typeof UIManager !== 'undefined') UIManager.saveFinal(); }
window.saveFinal = saveFinal;

function saveInvoiceData() { saveFinal(); }
window.saveInvoiceData = saveInvoiceData;

function markPaid() { if(typeof UIManager !== 'undefined') UIManager.markPaid(); }
window.markPaid = markPaid;

function mailInvoice() { if(typeof ExportManager !== 'undefined') ExportManager.mailInvoice(); }
window.mailInvoice = mailInvoice;

function shareInvoice() { if(typeof ExportManager !== 'undefined') ExportManager.shareInvoice(); }
window.shareInvoice = shareInvoice;

function saveState() { if(typeof StateManager !== 'undefined') StateManager.saveState(); }
window.saveState = saveState;

function undo() { if(typeof StateManager !== 'undefined') StateManager.undo(); }
window.undo = undo;

function redo() { if(typeof StateManager !== 'undefined') StateManager.redo(); }
window.redo = redo;

function restoreState(s) { if(typeof StateManager !== 'undefined') StateManager.restoreState(s); }
window.restoreState = restoreState;

function autoCalcDueDate() { if(typeof InvoiceEngine !== 'undefined') InvoiceEngine.autoCalcDueDate(); }
window.autoCalcDueDate = autoCalcDueDate;

function generateInvoiceNumber() { if(typeof InvoiceEngine !== 'undefined') InvoiceEngine.generateInvoiceNumber(); }
window.generateInvoiceNumber = generateInvoiceNumber;

function formatDate(d) { return typeof Utility !== 'undefined' ? Utility.formatDate(d) : ''; }
window.formatDate = formatDate;

function numberToWords(n) { return typeof Utility !== 'undefined' ? Utility.numberToWords(n) : ''; }
window.numberToWords = numberToWords;

function setupDragAndDrop() { if(typeof UIManager !== 'undefined') UIManager.setupDragAndDrop(); }
window.setupDragAndDrop = setupDragAndDrop;

function renderItems() { if(typeof UIManager !== 'undefined') UIManager.renderItems(); }
window.renderItems = renderItems;

function addItem() { if(typeof InvoiceEngine !== 'undefined') InvoiceEngine.addItem(); }
window.addItem = addItem;

function deleteItem(idx) { if(typeof InvoiceEngine !== 'undefined') InvoiceEngine.deleteItem(idx); }
window.deleteItem = deleteItem;

function clearItems() { if(typeof InvoiceEngine !== 'undefined') InvoiceEngine.clearItems(); }
window.clearItems = clearItems;

function updateItem(idx, field, val) { if(typeof InvoiceEngine !== 'undefined') InvoiceEngine.updateItem(idx, field, val); }
window.updateItem = updateItem;

function renderPaymentFields() { if(typeof PaymentManager !== 'undefined') PaymentManager.renderPaymentFields(); }
window.renderPaymentFields = renderPaymentFields;

function sync() { if(typeof InvoiceEngine !== 'undefined') InvoiceEngine.sync(); }
window.sync = sync;

function generateCodes() { if(typeof PaymentManager !== 'undefined') PaymentManager.renderQRCode(); }
window.generateCodes = generateCodes;

function handleQRUpload(input) { if(typeof PaymentManager !== 'undefined') PaymentManager.handleQRUpload(input); }
window.handleQRUpload = handleQRUpload;

function applyBranding() { if(typeof UIManager !== 'undefined') UIManager.applyBranding(); }
window.applyBranding = applyBranding;

function handleImageUpload(input, imgId) { if(typeof UIManager !== 'undefined') UIManager.handleImageUpload(input, imgId); }
window.handleImageUpload = handleImageUpload;

function generatePDF() { if(typeof ExportManager !== 'undefined') ExportManager.generatePDF(); }
window.generatePDF = generatePDF;

function printInvoice() { if(typeof PrintManager !== 'undefined') PrintManager.printInvoice(); }
window.printInvoice = printInvoice;

function exportCSV() { if(typeof ExportManager !== 'undefined') ExportManager.exportCSV(); }
window.exportCSV = exportCSV;

function exportFullDatabase() { if(typeof ExportManager !== 'undefined') ExportManager.exportFullDatabase(); }
window.exportFullDatabase = exportFullDatabase;

function importFullDatabase(e) { if(typeof ExportManager !== 'undefined') ExportManager.importFullDatabase(e); }
window.importFullDatabase = importFullDatabase;

function loadHistory() { if(typeof HistoryManager !== 'undefined') HistoryManager.loadHistory(); }
window.loadHistory = loadHistory;

function showHistory() { loadHistory(); }
window.showHistory = showHistory;

function openInvoice(id) { if(typeof HistoryManager !== 'undefined') HistoryManager.openInvoice(id); }
window.openInvoice = openInvoice;

function moveToTrash(id) { if(typeof HistoryManager !== 'undefined') HistoryManager.moveToTrash(id); }
window.moveToTrash = moveToTrash;

function restoreFromTrash(id) { if(typeof HistoryManager !== 'undefined') HistoryManager.restoreFromTrash(id); }
window.restoreFromTrash = restoreFromTrash;

function deletePermanently(id) { if(typeof HistoryManager !== 'undefined') HistoryManager.deletePermanently(id); }
window.deletePermanently = deletePermanently;

function duplicateInvoice(id) { if(typeof HistoryManager !== 'undefined') HistoryManager.duplicateInvoice(id); }
...
function saveProfile(type) { 
    if(typeof EntityManager !== 'undefined') {
        ...
        showToast('Database module not fully loaded.', 'error');
    }
}
...
window._fallbackItems = window._fallbackItems || [];
window.items = window._fallbackItems;

/* ================= SAFE LOGGER FALLBACK ================= */
var SafeLogger = {
    info: function(msg, data) { if(typeof Logger !== 'undefined') Logger.info(msg, data); else console.log('INFO:', msg, data || ''); },
    warn: function(msg, data) { if(typeof Logger !== 'undefined') Logger.warn(msg, data); else console.warn('WARN:', msg, data || ''); },
    error: function(msg, data) { if(typeof Logger !== 'undefined') Logger.error(msg, data); else console.error('ERROR:', msg, data || ''); }
};
...
if (typeof window.Utility === 'undefined') {
    window.Utility = {
        ...
        getVal: function(id) { const el = document.getElementById(id); return el ? el.value : ''; },
        formatCurrency: function(num) { return Number(num).toFixed(2); },
        formatDate: function(d) { if(!d) return ''; const date = new Date(d); return date.toLocaleDateString(); },
        numberToWords: function(n) { return n + " in words"; },
        generateId: function() { return 'INV-' + Date.now(); },
        debounce: function(func, wait) { let t; return function(...args) { clearTimeout(t); t = setTimeout(() => func.apply(this, args), wait); }; }
    };
}
...
var InvoiceEngine = {
    ...
    clearItems: function() {  
        ...
        if(confirm("Clear all items?")) { 
            ...
            window.items = (typeof StateManager !== 'undefined' && StateManager.items) ? StateManager.items : window._fallbackItems; 
            InvoiceEngine.addItem(); 
        }  
    },  

    updateItem: function(idx, field, val) {  
        let items = (typeof StateManager !== 'undefined' && StateManager.items) ? StateManager.items : window._fallbackItems;
    ...
    saveInvoiceAction: async function(status) {  
...
InvoiceEngine.syncDebounced = typeof Utility !== 'undefined' && Utility.debounce ? Utility.debounce(InvoiceEngine.sync, 150) : function() { setTimeout(InvoiceEngine.sync, 150); };
...
/* ================= UI MANAGER ================= */
var UIManager = {
    toggleSidebar: function() { const s = document.getElementById('app-sidebar'); if (s) s.classList.toggle('hidden'); },
    ...
    loadThemePersistence: async function() {
        if (typeof StorageEngine === 'undefined') return;
    ...
    saveDraft: function() { InvoiceEngine.saveInvoiceAction('Draft'); },
    saveFinal: function() { InvoiceEngine.saveInvoiceAction('Final'); },
    markPaid: function() { InvoiceEngine.saveInvoiceAction('Paid'); },
    ...
    handleImageUpload: function(input, imgId) {  
    ...
    setLoadingState: function(isLoading, text = 'Loading...') {  
...
var ExportManager = {
    ...
    importFullDatabase: async function(event) {
        if (typeof StorageEngine === 'undefined') return;
...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        SafeLogger.error('Initialization Failed:', error);
    }
});
