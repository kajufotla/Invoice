...
// Inject specific print and page-break styles for premium multi-page support  
            ...
            // Inject floating print button (hidden from print media via CSS .no-print logic built into Tailwind 'print:hidden')  
...
if(document.getElementById('prev-sender')) {  
    ...
    // Render stylized font for company name seamlessly  
...
if(document.getElementById('prev-items-body')) {  
    document.getElementById('prev-items-body').innerHTML = store.state.items.filter(i => i.desc || i.price > 0).map(item => `  
        <!-- avoid-break class prevents rows from splitting across pages -->  
...
if(btnPdf) {  
    btnPdf.addEventListener('click', async () => {  
        ...
        element.style.width = '800px'; // Maintains perfect document width ratio  

        // Enhanced options for high-quality export and page-break support  
        const options = {  
            margin: [10, 10, 10, 10],   
            ...
            pagebreak: { mode: 'css', avoid: '.avoid-break' } // Stops line items/totals from slicing in half  
        };  
